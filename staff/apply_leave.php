...
<?php
	...
	if($fromdate > $todate)
	{
	    echo "<script>alert('End Date cant precede Start Date');</script>";
	  }
	elseif($leave_days <= 0)
	{
	    echo "<script>alert('Leave limit exceeded, contact management');</script>";
	  }
		...
		$sql="INSERT INTO leave(LeaveType,ToDate,FromDate,Description,Status,IsRead,empid,num_days,PostingDate) VALUES(:leave_type,:fromdate,:todate,:description,:status,:isread,:empid,:num_days,:datePosting)";
		...
		if($lastInsertId)
		{
			echo "<script>alert('Leave Application successful.');</script>";
