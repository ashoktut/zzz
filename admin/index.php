				...
				<div class="col-lg-4 col-md-6 mb-20">
					<div class="card-box height-100-p pd-20 min-height-200px">
						...
						<div class="user-list">
							<ul>
								<?php
		                         $query = mysqli_query($conn,"select * from emp ORDER BY emp.emp_id desc limit 4") or die(mysqli_error());
				...
				<div class="col-lg-4 col-md-6 mb-20">
					<div class="card-box height-100-p pd-20 min-height-200px">
						...
						<div class="user-list">
							<ul>
								<?php
		                         $query = mysqli_query($conn,"select * from emp where role = 'Admin' ORDER BY emp.emp_id desc limit 4") or die(mysqli_error());
		                         while ($row = mysqli_fetch_array($query)) {
		                         $id = $row['emp_id'];
		                             ?>
					...
					<table class="data-table table stripe hover nowrap">
						...
						<tbody>
							<tr>
								...
								<?php $sql = "SELECT leave.id as lid,emp.FirstName,emp.LastName,emp.emp_id,leave.LeaveType,leave.PostingDate,leave.Status from leave join emp on leave.empid=emp.emp_id order by lid desc limit 5";
									$query = $dbh -> prepare($sql);
									$query->execute();
									$results=$query->fetchAll(PDO::FETCH_OBJ);
									$cnt=1;
