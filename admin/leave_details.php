...
<?php
	...
	{ 
		...
		if ($status === '2') {
			$sql="update leave set registra_remarks=:description,admin_status=:status,AdminRemarkDate=:admremarkdate where id=:did";
		...
		elseif ($status === '1') {
				$result = mysqli_query($conn,"update leave, emp set leave.registra_remarks='$description',leave.admin_status='$status',leave.AdminRemarkDate='$admremarkdate', emp.Av_leave='$REMLEAVE' where leave.empid = emp.emp_id AND leave.id='$did'");
