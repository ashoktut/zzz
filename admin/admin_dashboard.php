				...
				<div class="col-lg-4 col-md-6 mb-20">
					<div class="card-box height-100-p pd-20 min-height-200px">
						...
						<div class="user-list">
							<ul>
								<?php
		                         $query = mysqli_query($conn,"select * from emp where role = 'Staff' ORDER BY emp.emp_id desc limit 4") or die(mysqli_error());
		                         while ($row = mysqli_fetch_array($query)) {
		                         $id = $row['emp_id'];
		                             ?>
					...
					<table class="data-table table stripe hover nowrap">
						...
						<tbody>
							<tr>
								...
								<?php 
								$status=1;
								$sql = "SELECT leave.id as lid,emp.FirstName,emp.LastName,emp.location,emp.emp_id,leave.LeaveType,leave.PostingDate,leave.Status, leave.admin_status from leave join emp on leave.empid=emp.emp_id where leave.Status= '$status' order by lid desc limit 5";
